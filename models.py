...
class EtfModel(Base):
    """Represent static ETF instrument data."""
...
class PriceModel(Base):
    """Represent cached historical ETF close prices."""

    __tablename__ = "prices"
    __table_args__ = (UniqueConstraint("secid", "date", name="uq_prices_secid_date"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    secid: Mapped[str] = mapped_column(String(32), ForeignKey("etfs.secid", ondelete="CASCADE"), nullable=False)
...
class DividendModel(Base):
    """Represent cached ETF dividend payouts."""
...
class EtfMetricModel(Base):
    """Represent latest calculated ETF metrics for ranking."""

    __tablename__ = "etf_metrics"

    secid: Mapped[str] = mapped_column(String(32), ForeignKey("etfs.secid", ondelete="CASCADE"), primary_key=True)
...
class AllowedUserModel(Base):
    """Represent usernames that can trigger full data updates."""
